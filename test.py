...
for epoch in range(epochs):
    ...
    if val_loss < best_val:
        ...
        best_val = val_loss
        counter = 0
        torch.save(model.state_dict(),"best_delta_model.pt")
...
# 11 FORECAST
# -----------------------------
model.load_state_dict(torch.load("best_delta_model.pt"))
model.eval()
...
plt.plot(
    np.arange(len(series_smooth),len(series_smooth)+future_steps),
    future_series,
    label="Forecast"
)
